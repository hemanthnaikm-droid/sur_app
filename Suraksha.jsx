...
/* ══ FLOATING SOS BUTTON ══ */
.fab-sos{position:fixed;bottom:90px;right:18px;z-index:90;width:58px;height:58px;border-radius:50%;border:none;cursor:pointer;background:linear-gradient(135deg,#dc2626,#b91c1c);box-shadow:0 8px 24px rgba(220,38,38,.5),0 0 0 0 rgba(220,38,38,.6);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;animation:fabPulse 2.2s ease-in-out infinite;transition:transform .15s}
.fab-sos:active{transform:scale(.92)}
.fab-sos .fab-icon{font-size:1.4rem;line-height:1}
.fab-sos .fab-label{font-size:.5rem;font-weight:800;letter-spacing:.06em;margin-top:1px}
@keyframes fabPulse{0%,100%{box-shadow:0 8px 24px rgba(220,38,38,.5),0 0 0 0 rgba(220,38,38,.5)}50%{box-shadow:0 8px 24px rgba(220,38,38,.6),0 0 0 10px rgba(220,38,38,0)}}
.fab-sos.armed{background:linear-gradient(135deg,#f59e0b,#d97706);animation:fabArmed .6s ease-in-out infinite}
@keyframes fabArmed{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}

/* ══ GLOBAL SOS OVERLAY (fires from anywhere — shake/hardware/fab) ══ */
.gsos-overlay{position:fixed;inset:0;z-index:500;background:rgba(5,10,20,.92);backdrop-filter:blur(12px);display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:2rem;animation:fadeUp .2s ease-out}
.gsos-source{font-size:.7rem;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,.4);margin-bottom:.6rem;display:flex;align-items:center;gap:6px}
.gsos-source span.dot{width:6px;height:6px;border-radius:50%;background:#f59e0b;animation:pulse 1s infinite}
.gsos-big-num{font-size:5rem;font-weight:800;color:#f87171;font-family:'DM Sans',sans-serif;line-height:1;margin:.5rem 0}
.gsos-cancel{margin-top:1.5rem;padding:14px 36px;border-radius:14px;border:1.5px solid rgba(255,255,255,.25);background:rgba(255,255,255,.06);color:#fff;font-size:1rem;font-weight:600;cursor:pointer;transition:all .2s}
.gsos-cancel:hover{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.4)}
.gsos-sent-icon{font-size:4rem;margin-bottom:1rem}

/* ══ SHAKE-TO-SOS toggle row (Settings) ══ */
.shake-demo{display:flex;align-items:center;justify-content:center;gap:10px;padding:1rem;background:rgba(245,158,11,.06);border:1px solid rgba(245,158,11,.18);border-radius:12px;margin-bottom:.9rem}
.shake-demo-icon{font-size:1.6rem;animation:shakeWiggle 1.2s ease-in-out infinite}
@keyframes shakeWiggle{0%,100%{transform:rotate(0)}20%{transform:rotate(-12deg)}40%{transform:rotate(10deg)}60%{transform:rotate(-8deg)}80%{transform:rotate(6deg)}}

/* ══ HARDWARE DEVICE CARD ══ */
.hw-card{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:13px;padding:1rem 1.1rem;margin-bottom:.7rem;display:flex;align-items:center;justify-content:space-between;gap:10px}
.hw-card-l{display:flex;align-items:center;gap:11px}
.hw-icon{width:40px;height:40px;border-radius:10px;background:rgba(34,197,94,.15);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0}
.hw-icon.offline{background:rgba(255,255,255,.06)}
.hw-name{font-size:.84rem;font-weight:600}
.hw-meta{font-size:.68rem;color:rgba(255,255,255,.4);margin-top:1px}
.hw-status{font-size:.62rem;font-weight:700;padding:3px 9px;border-radius:20px;text-transform:uppercase;letter-spacing:.04em}
.hw-status.online{background:rgba(34,197,94,.18);color:#86efac;border:1px solid rgba(34,197,94,.3)}
.hw-status.offline{background:rgba(255,255,255,.08);color:rgba(255,255,255,.4);border:1px solid rgba(255,255,255,.12)}
.hw-code-box{background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:12px 14px;font-family:monospace;font-size:.7rem;color:#86efac;overflow-x:auto;white-space:pre;margin:.6rem 0;line-height:1.6}
.hw-copy-btn{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);color:#fff;padding:5px 11px;border-radius:7px;font-size:.68rem;cursor:pointer;transition:all .2s}
.hw-copy-btn:hover{background:rgba(255,255,255,.15)}
.hw-token-box{display:flex;align-items:center;gap:8px;background:rgba(124,58,237,.08);border:1px solid rgba(124,58,237,.2);border-radius:10px;padding:10px 13px;margin-bottom:.9rem}
.hw-token-val{flex:1;font-family:monospace;font-size:.74rem;color:#c4b5fd;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
`;
...
// ══════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════
//  GLOBAL SOS ENGINE — shared by Button / Shake / Hardware triggers
//  All three trigger paths call the SAME fireGlobalSOS() function,
//  so behaviour (WhatsApp + tel + email + Firestore log) is identical
//  no matter what triggered it.
// ══════════════════════════════════════════════════════════════
function useSOSEngine({ user, contacts, currentLocation, addToast }) {
  const [gState, setGState]   = useState("idle");   // idle | countdown | sent
  const [gCount, setGCount]   = useState(5);
  const [gSource, setGSource] = useState(null);      // "button" | "shake" | "hardware"
  const [gLog, setGLog]       = useState([]);
  const timerRef = useRef(null);

  const buildLocationMsg = () => {
    if (!currentLocation) return "Location unavailable — open Suraksha Location tab";
    return `https://maps.google.com/maps?q=${currentLocation.lat},${currentLocation.lng}`;
  };

  const logToFirestore = async (source) => {
    if (!user) return;
    try {
      await addDoc(collection(db, "sosLogs"), {
        uid: user.uid,
        triggeredAt: serverTimestamp(),
        triggerSource: source || "button",
  ...
  const fireGlobalSOS = async (source = "button") => {
    ...
    await logToFirestore(source);
    log.push(`🔥 Logged to Firebase (source: ${source})`);

    setGLog(log);
    setGState("sent");
    addToast("🚨 SOS Sent!", `Triggered via ${source}. ${contacts.length} contact${contacts.length !== 1 ? "s" : ""} alerted.`, "success");
    setTimeout(() => { setGState("idle"); setGSource(null); }, 8000);
  };

  const startGlobalSOS = (source = "button") => {
  ...
  return { gState, gCount, gSource, gLog, startGlobalSOS, cancelGlobalSOS, buildLocationMsg };
}

// ══════════════════════════════════════════════════════════════
//  SHAKE-TO-SOS — DeviceMotion API
//  3 hard shakes within 1.5s triggers SOS (configurable sensitivity)
// ══════════════════════════════════════════════════════════════
function useShakeDetection(onShakeDetected, enabled) {
  const lastUpdateRef  = useRef(0);
  const lastXRef       = useRef(0);
  const lastYRef       = useRef(0);
  const lastZRef       = useRef(0);
  const shakeCountRef  = useRef(0);
  const shakeWindowRef = useRef(0);
  const SHAKE_THRESHOLD = 18; // m/s² delta — tune for sensitivity
  ...
  useEffect(() => {
    ...
    const handleMotion = (event) => {
      ...
      const diffTime = now - lastUpdateRef.current;
      ...
      if (speed > SHAKE_THRESHOLD) {
        if (now - shakeWindowRef.current > WINDOW_MS) {
          shakeCountRef.current = 0;
          shakeWindowRef.current = now;
        }
        shakeCountRef.current += 1;
        if (shakeCountRef.current >= SHAKES_NEEDED) {
          shakeCountRef.current = 0;
          onShakeDetected();
        }
      }
      lastXRef.current = x; lastYRef.current = y; lastZRef.current = z;
    };

    window.addEventListener("devicemotion", handleMotion);
    return () => window.removeEventListener("devicemotion", handleMotion);
  }, [enabled, onShakeDetected]);
  ...
  return { requestShakePermission };
}

// ══════════════════════════════════════════════════════════════
//  HARDWARE SOS (Arduino + ESP32) — Firestore remote-trigger listener
//  The ESP32 device POSTs to Firestore (via REST API or Cloud Function)
//  writing to: hardwareTriggers/{deviceId} → { triggered: true, ts }
//  This hook listens in real-time via onSnapshot and fires SOS instantly.
// ══════════════════════════════════════════════════════════════
function useHardwareSOS(user, onHardwareTrigger) {
  const [deviceStatus, setDeviceStatus] = useState({}); // { deviceId: {online, lastSeen} }
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        const q = query(collection(db, "hardwareTriggers"), where("uid", "==", user.uid));
        unsub = onSnapshot(q, (snap) => {
          ...
          snap.docs.forEach(d => {
            ...
            const isOnline = Date.now() - lastSeenMs < 60000; // online if pinged in last 60s
            statusMap[deviceId] = { online: isOnline, lastSeen: data.lastSeen, deviceId, label: data.label || "ESP32 Device" };

            // Check for a NEW trigger event (debounced by timestamp)
            if (data.triggered && data.triggeredAt) {
              const triggerMs = data.triggeredAt?.toMillis?.() || 0;
              if (triggerMs > (lastTriggerRef.current[deviceId] || 0)) {
                lastTriggerRef.current[deviceId] = triggerMs;
                onHardwareTrigger(deviceId, data.label || "ESP32 Device");
              }
            }
          });
          setDeviceStatus(statusMap);
        });
      } catch (e) { console.warn("Hardware listener failed:", e.message); }
    };
    setup();
    return () => { if (unsub) unsub(); };
  }, [user?.uid]);

  const registerDevice = async (deviceId, label) => {
    try {
      const { setDoc, doc: fsDoc } = await import("firebase/firestore");
      await setDoc(fsDoc(db, "hardwareTriggers", `${user.uid}_${deviceId}`), {
        uid: user.uid,
        deviceId,
        label: label || "ESP32 Suraksha Button",
        triggered: false,
  ...
  return { deviceStatus, registerDevice };
}
...
// ══════════════════════════════════════════════════════════════
//  GLOBAL SOS OVERLAY — full-screen, fires from ANY trigger source
//  (floating button / shake / hardware) regardless of which module
//  is currently open. Sits above everything else in the app.
// ══════════════════════════════════════════════════════════════
function GlobalSOSOverlay({ gState, gCount, gSource, gLog, onCancel, contactsCount }) {
  if (gState === "idle") return null;

  const sourceLabel = { button: "📱 Floating Button", shake: "📳 Shake Detected", hardware: "🔌 Hardware Button" }[gSource] || "Manual Trigger";

  return (
    <div className="gsos-overlay">
      {gState === "countdown" && (
        <>
          <div className="gsos-source"><span className="dot" />{sourceLabel}</div>
          <div style={{ fontSize: "1.1rem", color: "rgba(255,255,255,.8)", fontWeight: 600 }}>Sending SOS Alert</div>
          <div className="gsos-big-num">{gCount}</div>
          <div style={{ fontSize: ".82rem", color: "rgba(255,255,255,.45)", marginBottom: "1rem" }}>
            Calling 112 · Alerting {contactsCount} contact{contactsCount !== 1 ? "s" : ""} · Logging location
          </div>
          <button className="gsos-cancel" onClick={onCancel}>✕ Cancel SOS</button>
        </>
      )}
      {gState === "sent" && (
        <>
          <div className="gsos-sent-icon">✅</div>
          <div style={{ fontSize: "1.15rem", fontWeight: 700, color: "#86efac", marginBottom: ".5rem" }}>SOS Alert Sent</div>
          <div style={{ fontSize: ".78rem", color: "rgba(255,255,255,.5)", marginBottom: "1rem" }}>Triggered via {sourceLabel}</div>
          <div style={{ textAlign: "left", maxWidth: 320 }}>
            {gLog.map((l, i) => (
              <div key={i} style={{ fontSize: ".75rem", color: "rgba(255,255,255,.6)", marginTop: 6 }}>✓ {l}</div>
            ))}
          </div>
        </>
      )}
...
// ══════════════════════════════════════════════════════════════
//  FLOATING SOS BUTTON — persistent across every screen
//  Single tap on the FAB triggers the SAME SOS engine as Shake/Hardware
// ══════════════════════════════════════════════════════════════
function FloatingSOSButton({ onTrigger, armed }) {
  return (
    <button className={`fab-sos ${armed ? "armed" : ""}`} onClick={onTrigger} title="Emergency SOS">
      <span className="fab-icon">🆘</span>
      <span className="fab-label">SOS</span>
    </button>
  );
}
...
// ══════════════════════════════════════════════════════════════
//  MODULE: HARDWARE SOS — Arduino / ESP32 Pairing & Setup
// ══════════════════════════════════════════════════════════════
function HardwareModule({ user, deviceStatus, registerDevice, onClose, addToast }) {
  const [pairing, setPairing]   = useState(false);
  const [deviceLabel, setDeviceLabel] = useState("ESP32 Wristband");
  const [showCode, setShowCode] = useState(false);
  const deviceId = user?.uid ? `dev_${user.uid.slice(0, 8)}` : "dev_xxxxxxxx";
  const apiEndpoint = `https://firestore.googleapis.com/v1/projects/suraksha-33bb4/databases/(default)/documents/hardwareTriggers/${user?.uid}_${deviceId}`;

  const devices = Object.values(deviceStatus || {});

  const pairNewDevice = async () => {
    setPairing(true);
    const ok = await registerDevice(deviceId, deviceLabel);
    setPairing(false);
    if (ok) addToast("🔌 Device Registered", `${deviceLabel} is now linked to your account.`, "success");
    else addToast("⚠️ Pairing Failed", "Could not register device. Check Firestore connection.", "error");
  };
  ...
  const arduinoCode = `// ═══ Suraksha SOS Button — ESP32 Firmware ═══
// Wire: Push button → GPIO 4 (INPUT_PULLUP), GND
#include <WiFi.h>
#include <HTTPClient.h>

const char* WIFI_SSID     = "YOUR_WIFI_NAME";
const char* WIFI_PASS     = "YOUR_WIFI_PASSWORD";
const char* FIREBASE_HOST = "suraksha-33bb4-default-rtdb.firebaseio.com";
const char* DEVICE_ID     = "${deviceId}";
const char* USER_UID      = "${user?.uid || 'YOUR_USER_UID'}";
...
void setup() {
  ...
  Serial.println("Suraksha SOS device online.");
}

void sendHeartbeat() {
  HTTPClient http;
  String url = "https://" + String(FIREBASE_HOST) +
    "/hardwareTriggers/" + String(USER_UID) + "_" + String(DEVICE_ID) + ".json";
  http.begin(url);
  http.addHeader("Content-Type", "application/json");
  String body = "{\"uid\":\"" + String(USER_UID) +
    "\",\"deviceId\":\"" + String(DEVICE_ID) +
    "\",\"lastSeen\":{\".sv\":\"timestamp\"},\"triggered\":false}";
  http.PATCH(body);
  http.end();
}
...
void sendSOSTrigger() {
  ...
  String body = "{\"uid\":\"" + String(USER_UID) +
    "\",\"deviceId\":\"" + String(DEVICE_ID) +
    "\",\"triggered\":true,\"triggeredAt\":{\".sv\":\"timestamp\"}}";
  http.PATCH(body);
  http.end();
...
void loop() {
  if (digitalRead(BUTTON_PIN) == LOW) {   // button pressed
    sendSOSTrigger();
    delay(3000);                          // debounce — avoid spamming
  }
  if (millis() - lastBeat > 30000) {       // heartbeat every 30s
    sendHeartbeat();
    lastBeat = millis();
  }
}`;

  const arduinoCircuit = `Components needed:
• ESP32 Dev Board (any variant)
• 1x Push button (momentary)
• 1x 10kΩ resistor (optional, INPUT_PULLUP used instead)
• Jumper wires
...
Optional additions:
  • Vibration motor on GPIO 5 → haptic confirm on press
  • RGB LED on GPIO 2 → red blink while sending, green on success
  • LiPo battery + TP4056 charge module for wearable use`;

  return (
    <div className="mp anim">
      <ModuleHeader icon="🔌" ci="sos" title="Hardware SOS Button" sub="Arduino / ESP32 wearable panic button" onClose={onClose} />

      <div className="sl">Paired Devices</div>
      {devices.length === 0 ? (
        <EmptyState icon="🔌" text="No hardware devices paired yet." sub="Register your ESP32 device below to enable a physical panic button." />
      ) : (
        devices.map(d => (
          <div key={d.deviceId} className="hw-card">
            <div className="hw-card-l">
              <div className={`hw-icon ${d.online ? "" : "offline"}`}>{d.online ? "🟢" : "⚪"}</div>
              <div>
                <div className="hw-name">{d.label}</div>
                <div className="hw-meta">ID: {d.deviceId}</div>
              </div>
            </div>
            <span className={`hw-status ${d.online ? "online" : "offline"}`}>{d.online ? "Online" : "Offline"}</span>
          </div>
        ))
      )}
      ...
      <div className="cf-box">
        ...
        <div className="fg">
          ...
          <input value={deviceLabel} onChange={e => setDeviceLabel(e.target.value)} placeholder="e.g. ESP32 Wristband, Keychain Button…" />
        </div>
        <button className="form-btn" onClick={pairNewDevice} disabled={pairing}>
          {pairing ? <><Spinner /> Registering…</> : "🔌 Register Device"}
        </button>
      </div>

      <div className="hw-token-box">
        <span style={{ fontSize: ".7rem", color: "rgba(255,255,255,.5)", flexShrink: 0 }}>Your User UID:</span>
        <span className="hw-token-val">{user?.uid || "Sign in to see UID"}</span>
        <button className="hw-copy-btn" onClick={() => copyCode(user?.uid || "")}>Copy</button>
      </div>

      <button className="share-btn" onClick={() => setShowCode(!showCode)}>
        {showCode ? "▼ Hide" : "▶ Show"} ESP32 Arduino Code & Wiring Guide
      </button>
      ...
      {showCode && (
        <>
          <div className="sl" style={{ marginTop: "1rem" }}>
            ...
            <button className="hw-copy-btn" style={{ marginLeft: 8 }} onClick={() => copyCode(arduinoCode)}>📋 Copy Code</button>
          </div>
          <div className="hw-code-box">{arduinoCode}</div>

          <div className="sl" style={{ marginTop: "1rem" }}>
            Wiring Guide
            <button className="hw-copy-btn" style={{ marginLeft: 8 }} onClick={() => copyCode(arduinoCircuit)}>📋 Copy</button>
          </div>
          <div className="hw-code-box">{arduinoCircuit}</div>
        </>
      )}

      <div className="divider" />
      <p className="hint">
        The ESP32 sends an HTTPS PATCH request to Firebase Realtime Database when the button is pressed.
        Suraksha listens via Firestore <code style={{color:"#c4b5fd"}}>onSnapshot</code> and fires the same SOS engine as the in-app button — instantly, with no app interaction needed.
      </p>
    </div>
  );
}

function ProfileModule({ user, contacts, recCount, onLogout, onClose, addToast, shakeEnabled, onEnableShake, onDisableShake }) {
  ...
  return (
    <div className="mp anim">
      ...
      {/* Settings */}
      <div className="sl">SOS Trigger Methods</div>
      <div className="shake-demo">
        <span className="shake-demo-icon">📳</span>
        <div style={{ flex: 1, textAlign: "left" }}>
          <div style={{ fontSize: ".82rem", fontWeight: 600 }}>Shake-to-SOS</div>
          <div style={{ fontSize: ".68rem", color: "rgba(255,255,255,.45)" }}>Shake your phone 3× quickly to trigger emergency SOS</div>
        </div>
        <Toggle on={shakeEnabled} onClick={() => shakeEnabled ? onDisableShake() : onEnableShake()} />
      </div>
...
function Dashboard({ user, onLogout }) {
  ...
  const [shakeEnabled, setShakeEnabled]   = useState(() => localStorage.getItem("sk_shake_enabled") === "true");
  const [shakePermGranted, setShakePermGranted] = useState(false);
  const { toasts, addToast }              = useToast();

  // ── Global SOS Engine — shared by Floating Button / Shake / Hardware ──
  const sosEngine = useSOSEngine({ user, contacts, currentLocation, addToast });

  // ── Shake-to-SOS ──
  const handleShake = useCallback(() => {
    if (sosEngine.gState === "idle") {
      sosEngine.startGlobalSOS("shake");
    }
  }, [sosEngine.gState]);
  const { requestShakePermission } = useShakeDetection(handleShake, shakeEnabled);

  const enableShake = async () => {
    const granted = await requestShakePermission();
    if (granted) {
      setShakeEnabled(true);
      setShakePermGranted(true);
      localStorage.setItem("sk_shake_enabled", "true");
      addToast("📳 Shake-to-SOS Enabled", "Shake your phone 3 times quickly to trigger SOS.", "success");
    } else {
      addToast("⚠️ Permission Denied", "Motion sensor access denied. Enable in device settings.", "error");
    }
  };
  const disableShake = () => {
    setShakeEnabled(false);
    localStorage.setItem("sk_shake_enabled", "false");
    addToast("📳 Shake-to-SOS Disabled", "Shake detection turned off.", "info");
  };

  // ── Hardware SOS (Arduino/ESP32) ──
  const handleHardwareTrigger = useCallback((deviceId, label) => {
    if (sosEngine.gState === "idle") {
      addToast("🔌 Hardware Trigger Detected", `${label} activated SOS.`, "info");
      sosEngine.startGlobalSOS("hardware");
    }
  }, [sosEngine.gState]);
  const { deviceStatus, registerDevice } = useHardwareSOS(user, handleHardwareTrigger);
  ...
  const cards = [
    ...
    { id: "hardware",  cls: "c-sos",   icon: "🔌", ci: "sos",   title: "Hardware SOS Button",  desc: `${Object.keys(deviceStatus).length} ESP32 device${Object.keys(deviceStatus).length!==1?"s":""} paired. Physical panic button.`, badge: "NEW" },
  ];
        ...
        <div className="mp-wrap" onClick={e => e.target === e.currentTarget && closeModule()}>
          ...
          {module === "profile"   && (
            <ProfileModule
              ...
              onDisableShake={disableShake}
            />
          )}
        </div>
      )}

      {/* ── Floating SOS Button — visible on every screen, every module ── */}
      <FloatingSOSButton
        armed={sosEngine.gState === "countdown"}
        onTrigger={() => sosEngine.startGlobalSOS("button")}
      />

      {/* ── Global SOS Overlay — fires from Floating Button / Shake / Hardware ── */}
      <GlobalSOSOverlay
        gState={sosEngine.gState}
        gCount={sosEngine.gCount}
        gSource={sosEngine.gSource}
        gLog={sosEngine.gLog}
        onCancel={sosEngine.cancelGlobalSOS}
